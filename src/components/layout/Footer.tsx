    ...
    <footer className="bg-[var(--color-bg-card)] border-t border-[var(--color-border)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-16">
          <div>
            <h3 className="text-lg font-medium text-[var(--color-text)] mb-4 font-serif">Contact</h3>
            <ul className="space-y-3">
              <li>
                <a href={`tel:${formatPhone(contactInfo.phone)}`} className="text-[var(--color-textMuted)] hover:text-[var(--color-accent)] transition-colors">
                  {contactInfo.phone}
                </a>
              </li>
              <li>
              ...
              <li>
                <address className="text-[var(--color-textMuted)] not-italic">
          ...
          <div className="md:text-right">
            <h3 className="text-lg font-medium text-[var(--color-text)] mb-4 font-serif">Follow</h3>
            <div className="flex gap-4 md:justify-end mb-6">
              {socialLinks.map((social) => (
                <a key={social.platform} href={social.url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-md bg-[var(--color-border)] text-[var(--color-textMuted)] hover:bg-[var(--color-accent)] hover:text-[var(--color-primary)] transition-all duration-300" aria-label={social.label}>
            ...
            <button onClick={scrollToTop} className="inline-flex items-center gap-2 text-[var(--color-textMuted)] hover:text-[var(--color-accent)] transition-colors text-sm">
