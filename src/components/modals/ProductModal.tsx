    ...
    <Modal isOpen={isProductModalOpen} onClose={closeProductModal} size="xl">
      <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          {selectedProduct.images.length > 1 && (
            <div className="absolute bottom-4 left-4 right-4 flex gap-2 overflow-x-auto">
              {selectedProduct.images.map((img, i) => (
                <button key={i} onClick={() => setSelectedImageIndex(i)} className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${i === selectedImageIndex ? 'border-[var(--color-accent)]' : 'border-white/20 opacity-60 hover:opacity-100'}`}>
        ...
        <div>
          ...
          <ModalContent>
            <div className="text-xs text-[var(--color-accent)] uppercase tracking-wider mb-2">{selectedProduct.category}</div>
            <p className="text-[var(--color-textMuted)] mb-6 leading-relaxed">{selectedProduct.description}</p>
            {selectedProduct.features && selectedProduct.features.length > 0 && (
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-[var(--color-text)] mb-3">Key Features</h4>
                <ul className="space-y-2">
                  {selectedProduct.features.map((f, i) => <li key={i} className="flex items-start gap-2 text-[var(--color-textMuted)] text-sm"><svg className="w-4 h-4 text-[var(--color-accent)] mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{f}</li>)}
                </ul>
              </div>
            )}
